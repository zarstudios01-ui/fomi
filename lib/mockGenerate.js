...
function buildProxyImageUrl({ prompt, creativeDirection, variant, seed }) {
...
export async function mockGenerate({ prompt, creativeDirection, count = 4 }) {
  ...
  const variations = Array.from({ length: count }).map((_, i) => {
    ...
    return {
      ...
      engine: "Pollinations",
      imageUrl: buildProxyImageUrl({ prompt, creativeDirection, variant: variants[i % variants.length], seed }),
